...
namespace :versioned_unpublish do
  desc "Retire a document on GOV.UK e.g. versioned_unpublish:retire['a-content-id'] NOTE='A note'"
  task :retire, [:content_id] => :environment do |_, args|
    ...
    document = Versioned::Document.find_by!(content_id: args.content_id, locale: locale)
    raise "Document must have a published version before it can be retired" unless document.live_edition

    Versioned::UnpublishService.new.retire(document.live_edition, explanatory_note)
  end
  ...
  task :remove, [:content_id] => :environment do |_, args|
    ...
    Versioned::UnpublishService.new.remove(
