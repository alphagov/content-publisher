module Requirements
  class FileAttachmentRevisionChecker
    UNIQUE_REF_MAX_LENGTH = 255
    ISBN10_REGEX = /^(?:\d[\ -]?){9}[\dX]$/i.freeze
    ISBN13_REGEX = /^(?:\d[\ -]?){13}$/i.freeze

    attr_reader :file_attachment_revision

    def initialize(file_attachment_revision)
      @file_attachment_revision = file_attachment_revision
    end
    ...
    def pre_publish_issues
      ...
      if file_attachment_revision.official_document_type.blank?
        issues.create(:file_attachment_official_document_type,
                      :blank,
                      filename: file_attachment_revision.filename,
                      attachment_revision: file_attachment_revision)
      end
