...
RSpec.describe WhitehallImporter::Import do
  describe ".call" do
    ...
    it "creates a document" do
      expect { described_class.call(build(:whitehall_import)) }
    ...
    it "does not add users who have logged into Content Publisher" do
      ...
      expect { described_class.call(record) }.not_to(change { User.count })
    end
    ...
    it "sets created_by_id as the original author" do
      ...
      record = build(:whitehall_import, payload: import_data)
      document = described_class.call(record)
