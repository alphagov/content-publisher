RSpec.describe Requirements::Form::PublishTimeChecker do
  describe ".call" do
    it "returns no issues if there are none" do
      issues = described_class.call(1.day.from_now)
      expect(issues).to be_empty
    end

    it "returns a date issue if the date is in the past" do
      issues = described_class.call(1.day.ago)
    ...
    it "returns a time issue if just the time is in the past" do
      travel_to(Time.zone.parse("2019-06-17 15:00")) do
        issues = described_class.call(1.hour.ago)
