...
RSpec.describe Image::MetadataRevision do
  describe "#different_to?" do
    it "is true when metadata is different" do
      revision1 = build(:image_metadata_revision, alt_text: "this")
      revision2 = build(:image_metadata_revision, alt_text: "that")

      expect(revision1.different_to?(revision2)).to be true
    end

    it "is false when metadata is the same and only timestamps differ" do
      revision1 = build(:image_metadata_revision,
                        credit: "Getty",
                        created_at: 10.days.ago)
      revision2 = build(:image_metadata_revision,
                        credit: "Getty",
  ...
  describe "#build_revision_update" do
    let(:existing_revision) do
      create(:image_metadata_revision, caption: "Great shot")
    end
