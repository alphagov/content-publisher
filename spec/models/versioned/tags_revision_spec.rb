...
RSpec.describe Versioned::TagsRevision do
  describe "#different_to?" do
    it "is true when update data is different" do
      revision1 = build(:versioned_tags_revision, tags: { "a" => %w[a b] })
      revision2 = build(:versioned_tags_revision, tags: { "a" => %w[c d] })

      expect(revision1.different_to?(revision2)).to be true
    end

    it "is false when content is the same and only timestamps differ" do
      revision1 = build(:versioned_tags_revision,
                        tags: { "a" => %w[a b] },
                        created_at: 10.days.ago)
      revision2 = build(:versioned_tags_revision,
                        tags: { "a" => %w[a b] },
  ...
  describe "#build_revision_update" do
    let(:existing_revision) do
      create(:versioned_tags_revision)
    end
