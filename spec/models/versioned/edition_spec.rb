...
RSpec.describe Versioned::Edition do
  ...
  describe ".create_initial" do
    ...
    it "creates a current edition" do
      edition = Versioned::Edition.create_initial(document, user)

      expect(edition).to be_a(Versioned::Edition)
    ...
    it "has a revision" do
      ...
      expect(edition.revision).to be_a(Versioned::Revision)
      expect(edition.created_by).to eq(user)
    end
    ...
    it "has a status which is draft" do
      ...
      expect(edition.status).to be_a(Versioned::Status)
  ...
  describe ".create_next_edition" do
    ...
    it "creates a draft edition with next edition number" do
      preceding = create(:versioned_edition, number: 5, current: false)

      edition = Versioned::Edition.create_next_edition(preceding, user)
    ...
    it "resets the change note and update type" do
      preceding = create(:versioned_edition,
                         change_note: "Changes",
                         update_type: :minor,
                         current: false)
  ...
  describe "#resume_discarded" do
    let(:live_edition) { create(:versioned_edition, :published, current: false) }
    let(:user) { build(:user) }
  ...
  describe "#assign_status" do
    ...
    it "can set details on the status" do
      removal = build(:versioned_removal)
  ...
  describe "#assign_revision" do
    let(:edition) { build(:versioned_edition) }
    let(:revision) { build(:versioned_revision, document: edition.document) }
    let(:user) { build(:user) }
